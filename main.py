...
if not stops_df.empty:
    stops_gdf = gpd.GeoDataFrame(
        stops_df.copy(),
        geometry=gpd.points_from_xy(stops_df.stop_lon.astype(float), stops_df.stop_lat.astype(float)),
        crs="EPSG:4326"
    )
else:
...
if scenario.frequencies is not None and len(scenario.frequencies) > 0:
    ...
    freq = scenario.frequencies.merge(scenario.trips[['trip_id', 'route_id']], on='trip_id')

    schedule_headways = (
        freq.groupby('route_id')['headway_secs']
        .mean()
        .to_dict()
    )
...
class Vehicle:
    def __init__(self, vehicle_id: str, route: str, shape: List[tuple], trip_id: str = None, speed: float = 10.0):
...
class TickEngine:
    ...
    def run(self, max_ticks: int = 100, tick_interval_range=(1, 5)):
        for _ in range(max_ticks):
            ...
            self.performance_metrics['avg_tick_time'] = statistics.mean(self.performance_metrics['tick_times']) if self.performance_metrics['tick_times'] else 0.0
...
try:
    ...
    for v in vehicles:
        print(f"DEBUG vehicle {v.vehicle_id}: position_idx={v.position_idx}, finished={v.finished}, shape_len={len(v.shape)}")
    ...
    metrics_collector = MetricsCollector(schedule_headways=schedule_headways, schedule_df=stop_times)
    ...
    for tick in range(50):  # Increased to 50 ticks for better observation
        try:
            ...
            for v in engine.vehicles:
                ...
                metrics_collector.record_vehicle(v.vehicle_id, engine.current_tick, veh_metrics)
                ...
                if v.is_dwelling and (not was_dwelling):
                    ...
                    metrics_collector.record_stop(v, stop_id, sim_clock)   # ver cambios en collectors.py abajo
                    # log estructurado
                    log_event("stop", {
            ...
            metrics_collector.record_system(engine.current_tick, {
                "active_vehicles": active_vehicles,
