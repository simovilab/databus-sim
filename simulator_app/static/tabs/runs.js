...
function startPolling(runId) {
    ...
    const id = setInterval(async () => {
        ...
        try {
            ...
            // Any error (including 404) is transient under HTTP polling:
            // databus may not have populated the state row yet, or the
            // request failed in flight. Cancellation arrives as an
            // explicit "Cancelled" lifecycle state, not as a 404.
            return;
        }
...
async function clearCompleted() {
    ...
    const checks = await Promise.all(runs.map(async (r) => {
        if (TERMINAL_STATES.has(r.lifecycle_state)) return false;
        try {
            await simClient.getRun(r.run_id);
            return true;
        } catch (e) {
            // 404 means databus no longer knows this run (purged externally).
            // Any other error is transient — keep the run so it gets re-polled.
            return !e?.message?.includes('HTTP 404');
        }
    }));
    const kept = runs.filter((_, i) => checks[i]);
    for (const r of runs) {
        if (!kept.includes(r)) {
            stopPolling(r.run_id);
            _states.delete(r.run_id);
        }
    }
    localStorage.setItem(RECENT_RUNS_KEY, JSON.stringify(kept));
    buildRunList();
}
