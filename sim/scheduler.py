...
Owned by Agent B (B2). P0 fixes the file format (``CONTRACTS.md`` §3), the
public method names, and the interaction with :class:`DatabusClient` /
:class:`RunBinder`. B2 implements the loop.
"""
...
from .databus_client import CreateRunRequest, DatabusClient, DatabusError, UpdateRunRequest
...
class Scheduler:
    ...
    def load(self, path: Path | None = None) -> None:
        ...
        self._defaults = {
            "pre_run_idle_s": int(defaults.get("pre_run_idle_s", self.DEFAULT_PRE_RUN_IDLE_S)),
            "post_run_idle_s": int(defaults.get("post_run_idle_s", self.DEFAULT_POST_RUN_IDLE_S)),
    ...
    def reload(self) -> None:
        ...
        if self.state_publisher is not None:
            self.state_publisher.publish_schedule_snapshot(self.snapshot()["runs"])
    ...
    def snapshot(self) -> dict[str, Any]:
        """Return the payload for ``sim/state/schedule`` and ``GET /schedule``."""
    ...
    async def _dispatch_entry(self, entry: dict[str, Any]) -> None:
        ...
        log.info("scheduler.dispatch entry_id=%s vehicle_id=%s", eid, entry.get("vehicle_id"))
        ...
        if entry.get("auto_confirm"):
            delay_s = self._defaults.get("auto_confirm_delay_s", self.DEFAULT_AUTO_CONFIRM_DELAY_S)
